...
IAM permission over-provisioning detector.
...
Scans IAM policies for security findings.
...
- Does not remediate or modify IAM policies — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated IAM policies monitoring — point-in-time security audit only
...
```bash
iamspectre scan --output json | jq '.summary'
iamspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
