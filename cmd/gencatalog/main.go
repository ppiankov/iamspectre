...
// WO-64@v4: generation is an offline supply-chain step, never a scanner runtime dependency.
package main
...
// WO-64@v4: restrict provenance to AWS's documented service-reference host.
const serviceReferenceHost = "servicereference.us-east-1.amazonaws.com"

// WO-64@v4: model only the documented official service-reference schema.
...
// WO-64@v4: retain action resource evidence and accept documented ancillary fields.
type action struct {
	Name                string          `json:"Name"`
...
// WO-64@v4: resource presence is the only applicability fact published in this schema.
type resource struct {
	Name string `json:"Name"`
}

// WO-64@v4: bind a pinned fixture to its retrieval provenance outside AWS's payload.
type sourceMetadata struct {
	URL         string
	RetrievedAt string
}

// WO-64@v4: normalize source actions into deterministic generated entries.
type catalogEntry struct {
	Name          string
	Applicability string
}

// WO-64@v4: expose deterministic offline generation as an explicit build-time command.
func main() {
...
// WO-64@v4: reject upstream schema drift and ambiguous action records before emitting data.
...
// WO-64@v4: accept provenance only from the exact official per-service endpoint.
...
// WO-64@v4: reject concatenated JSON so the hashed fixture has one interpretation.
...
// WO-64@v4: sorted entries and explicit metadata make identical inputs byte-reproducible.
func render(source serviceReference, metadata sourceMetadata, digest string, entries []catalogEntry) ([]byte, error) {
	...
	fmt.Fprintln(&output)
	fmt.Fprintln(&output, "// WO-64@v4: expose the pinned evidence digest to catalog consumers.")
	fmt.Fprintf(&output, "const resourceApplicabilityCatalogDigest = %q\n", digest)
	fmt.Fprintln(&output)
	fmt.Fprintln(&output, "// WO-64@v4: resourceApplicabilityCatalog is pinned generated evidence, not runtime inference.")
	...
	fmt.Fprintln(&output, "}")
	fmt.Fprintln(&output)
	fmt.Fprintln(&output, "// WO-64@v4: ResourceApplicability reports only what the AWS JSON schema proves.")
...
// WO-64@v4: keep generator failures deterministic and non-recovering.
